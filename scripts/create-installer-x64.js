...
const perplexityAppPath = path.join(releaseBuildsDir, 'Perplexity-darwin-x64', 'Perplexity.app');
...
const applescriptPath = path.join(scriptsDir, 'InstallPerplexity-x64.applescript');
fs.writeFileSync(applescriptPath, appleScript);

// Compile the AppleScript
const installAppPath = path.join(scriptsDir, 'Perplexity-installer-x64.app');
...
//
// // Create the DMG
// execSync(`electron-installer-dmg "${perplexityAppPath}" Perplexity-installer-x64 --out="${releaseBuildsDir}" --overwrite --icon="${path.join(rootDir, 'icons', 'logo.icns')}" --contents="${dmgContentsPath}"`);
